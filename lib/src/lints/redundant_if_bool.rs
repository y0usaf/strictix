        ...
        let (message, replacement) = match (then_bool, else_bool) {
            ...
            (false, true) => {
                ...
                let negated = match cond {
                    Expr::Apply(_) | Expr::Paren(_) | Expr::Ident(_) | Expr::HasAttr(_) => {
                        make::unary_not(cond_node).syntax().clone()
                    }
                    _ => {
                        let parens = make::parenthesize(cond_node);
...
fn boolean_value(expr: &Expr) -> Option<bool> {
    ...
    match Ident::cast(ident.syntax().clone())?.to_string().as_str() {
        "true" => Some(true),
        "false" => Some(false),
        _ => None,
    }
