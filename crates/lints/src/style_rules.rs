...
use strictix_syntax::{
    AstNode, AttrItem, AttrName, BinExpr, Binding, Expr, IfExpr, InheritStmt, LambdaExpr,
...
impl Rule for EmptyLetIn {
    ...
    fn check_node(&self, node: &SyntaxNode, source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        let mut diag = Diagnostic::new(
            "empty-let-in",
            Severity::Warning,
            "useless let-in expression",
            node.content_range(),
        );
        let has_comments = node.child_tokens().any(|t| t.kind() == K::Comment);
        if !has_comments {
            diag = diag.with_fix(
                Fix::new("remove empty let").edit(node.content_range(), body.text(source)),
            );
        }
        diags.push(diag);
    }
...
impl Rule for ManualInherit {
    ...
    fn check_node(&self, node: &SyntaxNode, source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        let Some(AttrName::Ident(key)) = elems.next() else {
            return;
        };
        if elems.next().is_some() {
            return;
        }
        let Some(Expr::Ident(value)) = binding.value() else {
            return;
        };
        if key.text(source) != value.text(source) {
            return;
        }
        let fix = Fix::new("use inherit").edit(
            node.content_range(),
            format!("inherit {};", key.text(source)),
        );
        diags.push(
            Diagnostic::new(
                "manual-inherit",
...
impl Rule for ManualInheritFrom {
    ...
    fn name(&self) -> &'static str {
        "Assignment instead of inherit from"
    }
    fn description(&self) -> &'static str {
    ...
    fn check_node(&self, node: &SyntaxNode, source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        let Some(Expr::Select(select)) = binding.value() else {
            return;
        };
        ...
        let Some(select_attrpath) = select.attrpath() else {
            return;
        };
        ...
        if key.text(source) != index.text(source) {
            return;
        }
...
impl Rule for CollapsibleLetIn {
    ...
    fn check_node(&self, node: &SyntaxNode, _source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        let Some(Expr::Let(inner)) = let_expr.body() else {
            return;
        };
        let Some(in_token) = node.child_tokens().find(|t| t.kind() == K::KwIn) else {
            return;
        };
        let Some(let_token) = inner.syntax().child_tokens().find(|t| t.kind() == K::KwLet) else {
            return;
        };
        ...
        diags.push(
            Diagnostic::new(
                "collapsible-let-in",
                Severity::Warning,
                "these let-in expressions are collapsible",
                node.content_range(),
            )
            .with_fix(fix),
        );
...
impl Rule for EtaReduction {
    ...
    fn check_node(&self, node: &SyntaxNode, source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        let Some(Expr::Apply(apply)) = lambda.body() else {
            return;
        };
        let Some(Expr::Ident(arg)) = apply.arg() else {
            return;
        };
        if arg.text(source) != param.text(source) {
            return;
        }
        let Some(Expr::Ident(func)) = apply.func() else {
            return;
        };
        if func.text(source) == param.text(source) {
            return;
        }
        ...
        diags.push(
            Diagnostic::new(
                "eta-reduction",
                Severity::Warning,
                "this function is eta-reducible",
                node.content_range(),
            )
            .with_fix(fix),
        );
...
impl Rule for EmptyPattern {
    ...
    fn check_node(&self, node: &SyntaxNode, source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        let LambdaParam::Formals(formals, at_name) = lambda.param() else {
            return;
        };
        if formals.params().count() != 0 {
            return;
        }
        ...
        if is_module(lambda.body(), source) {
            return;
        }
        ...
        diags.push(
            Diagnostic::new(
                "empty-pattern",
                Severity::Warning,
                "empty pattern in function argument",
                node.content_range(),
            )
            .with_fix(fix),
        );
...
fn is_module(body: Option<Expr<'_>>, source: &str) -> bool {
    let Some(Expr::Attrset(attrset)) = body else {
        return false;
    };
    attrset.items().any(|item| {
        ...
        let Some(ap) = binding.attrpath() else {
            return false;
        };
...
impl Rule for RedundantPatternBind {
    ...
    fn check_node(&self, node: &SyntaxNode, source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        let start = at
            .range()
            .start()
            .min(formals.syntax().content_range().start());
        let end = at.range().end().max(formals.syntax().content_range().end());
        let fix =
            Fix::new("remove redundant pattern").edit(TextRange::new(start, end), at.text(source));
        diags.push(
            Diagnostic::new(
                "redundant-pattern-bind",
                Severity::Warning,
                "redundant pattern bind in function argument",
                node.content_range(),
            )
            .with_fix(fix),
        );
...
impl Rule for EmptyInherit {
    ...
    fn check_node(&self, node: &SyntaxNode, _source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        if inherit.source().is_some() {
            return;
        }
        if inherit.names().count() != 0 {
            return;
        }
...
impl Rule for DeprecatedToPath {
    ...
    fn check_node(&self, node: &SyntaxNode, source: &str, diags: &mut Vec<Diagnostic>) {
        let Some(apply) = strictix_syntax::ApplyExpr::cast(node) else {
            return;
        };
        let Some(func) = apply.func() else { return };
        let func_text = func.text(source);
        if func_text != "toPath" && func_text != "builtins.toPath" {
            return;
        }
...
impl Rule for UselessHasAttr {
    ...
    fn check_node(&self, node: &SyntaxNode, source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        let Some(Expr::HasAttr(has_attr)) = if_expr.cond() else {
            return;
        };
        let Some(set) = has_attr.base() else { return };
        let Some(attrpath) = has_attr.attrpath() else {
            return;
        };
        let Some(then_branch) = if_expr.then_branch() else {
            return;
        };
        if !matches!(then_branch, Expr::Select(_)) {
            return;
        }
        ...
        if then_branch.text(source).trim() != expected {
            return;
        }
        let Some(default_expr) = if_expr.else_branch() else {
            return;
        };
        ...
        diags.push(
            Diagnostic::new(
                "useless-has-attr",
                Severity::Warning,
                "this if-expression can be simplified with `or`",
                node.content_range(),
            )
            .with_fix(fix),
        );
...
impl Rule for EmptyListConcat {
    ...
    fn check_node(&self, node: &SyntaxNode, source: &str, diags: &mut Vec<Diagnostic>) {
        ...
        let fix =
            Fix::new("remove empty-list concat").edit(node.content_range(), survivor.text(source));
        diags.push(
            Diagnostic::new(
                "empty-list-concat",
                Severity::Warning,
...
impl Rule for UselessParens {
    ...
    fn check_file(&self, model: &SemanticModel, _config: &LintConfig, diags: &mut Vec<Diagnostic>) {
        ...
        walk(
            root,
            &mut ancestors,
            &mut |node, ancestors| match node.kind() {
                K::Binding => {
                    let Some(binding) = Binding::cast(node) else {
                        return;
                    };
                ...
                K::ParenExpr => {
                    if let Some(parent) = ancestors.last() {
                        if parent.kind() == K::Binding || parent.kind() == K::LetExpr {
                            return;
                        }
                    }
                    let Some(paren) = ParenExpr::cast(node) else {
                        return;
                    };
...
fn push_paren_diag(diags: &mut Vec<Diagnostic>, range: TextRange, inner: &str, message: &str) {
    diags.push(
        Diagnostic::new("useless-parens", Severity::Warning, message, range)
...
fn is_primitive(expr: Expr<'_>) -> bool {
    match expr {
        Expr::List(_) | Expr::Paren(_) | Expr::String(_) | Expr::Attrset(_) | Expr::Ident(_) => {
            true
        }
...
impl Rule for RepeatedKeys {
    ...
    fn check_file(&self, model: &SemanticModel, _config: &LintConfig, diags: &mut Vec<Diagnostic>) {
        ...
        walk(root, &mut ancestors, &mut |node, ancestors| {
            ...
            let mut elems = attrpath.elements();
            let Some(AttrName::Ident(first)) = elems.next() else {
                return;
            };
            ...
            if parent.kind() != K::AttrsetExpr {
                return;
            }
            if ancestors.len() >= 2 && ancestors[ancestors.len() - 2].kind() == K::RecAttrsetExpr {
                return;
            }
            let name = first.text(source);
            let attrset = strictix_syntax::AttrsetExpr::cast(parent).expect("kind checked");
            let mut count = 0usize;
            let mut distinct_second: Vec<&str> = Vec::new();
            ...
            for item in attrset.items() {
                if let AttrItem::Binding(b) = item {
                    if let Some(ap) = b.attrpath() {
                        let mut elements = ap.elements();
                        if let Some(AttrName::Ident(id)) = elements.next() {
                            if id.text(source) == name {
                                count += 1;
                                if first_range.is_none() {
                                    first_range = Some(b.range());
                                }
                                // A common nested prefix is a valid Nix merge:
                                // `finit.tasks."hermes-dirs".*` builds one tree.
                                // Only report when the second component actually
                                // branches, which is the flattening this rule targets.
                                if let Some(second) = elements.next() {
                                    let text = match second {
                                        AttrName::Ident(id) => id.text(source),
                                        _ => "",
                                    };
                                    if !distinct_second.contains(&text) {
                                        distinct_second.push(text);
                                    }
                                }
                            }
                        }
            ...
            if count < 3 || distinct_second.len() < 2 {
                return;
            }
            if first_range != Some(node.range()) {
                return;
            }
            diags.push(Diagnostic::new(
                "repeated-keys",
