...
mod json {
    ...
    impl JsonSpan {
        fn from_textrange(at: TextRange, line_index: &super::LineIndex) -> Self {
            let start = at.start();
            let end = at.end();
            let (from_line, from_column) = line_index.position(start);
            let (to_line, to_column) = line_index.position(end);
            let from = Position {
                line: from_line,
                column: from_column,
            };
            ...
            .map(|r| {
                ...
                let diagnostics = r
                    ...
                    .map(|d| JsonDiagnostic {
                        at: JsonSpan::from_textrange(d.at, &line_index),
                        message: &d.message,
                        suggestion: d.suggestion.as_ref().map(|s| JsonSuggestion {
                            at: JsonSpan::from_textrange(s.at, &line_index),
                            fix: s.fix.to_string(),
                        }),
                    })
                    .collect::<Vec<_>>();
...
impl LineIndex {
    ...
    fn position(&self, at: TextSize) -> (usize, usize) {
        let offset: usize = at.into();
